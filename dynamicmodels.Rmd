...
sobol_forest = sobol2007(model = NULL, X1, X2, nboot = 300)

sim_times = seq(from=1, to=300)
sobol_parms = list(closure = 100, K = 500, r=sobol_forest$X$sobol_r[1], g = sobol_forest$X$sobol_g[1], temp = 20)

sens_result = ode(y=C, times=sim_times, func=forestgrowthmodel, parms=sobol_parms)
...
compute_metrics = function(sens_result) {
  max_size = max(sens_result$forest_size)
return(list(maxsize=maxsize))}

#######################
# i feel good about the code up until this point, and then get a bit confused with the syntax for parms in the wrapper function and how to make allresults run without an error
######################

wrapper = function(r, g, C, sim_times, func) {
    parms = list(r=r, g=g)
    result = ode(y=C, times=sim_times, func=forestgrowthmodel, parms=parms) 
    colnames(result)=c("time","forest_size")
  # get metrics
  metrics=compute_metrics(as.data.frame(sens_result))
  return(metrics)
}
...
sens_tell = sensitivity::tell(sobol_forest, allres$maxsize)
...
sobol_forest_result_graph <- matrix((unlist(allresult)), ncol = 1, byrow = TRUE)
colnames(sobol_forest_result_graph) = c("forest_size")
